 ...
 */
select A.bbl, A.property_id, A.total_actual_emission
from derived_penalty_variables A
         left join feed_ll84_bbl B
                   on A.bbl = B.bbl
group by A.bbl, A.property_id, A.total_actual_emission
 ...
 */
select A.bbl, A.property_id, A.site_eui from feed_ll84_bbl A
 ...
 */
select A.bbl, A.property_id, A.emission_penalty_2024_2029
from derived_penalty_variables A
         left join feed_ll84_bbl B
                   on A.bbl = B.bbl
group by A.bbl, A.property_id, A.emission_penalty_2024_2029, A.phase1_penalty
 ...
 */
select A.bbl, A.property_id, A.emission_penalty_2030_2034
from derived_penalty_variables A
         left join feed_ll84_bbl B
                   on A.bbl = B.bbl
group by A.bbl, A.property_id, A.emission_penalty_2030_2034, A.phase2_penalty
 ...
 * TODO : dint understand the query
 */
 ...
 * - since penalty is calculated using combination of largest, second and third use types there is no way to map the value to each use type without a default weightage(say 33% for each)
 * - other option is to start computing & storing total emissions at a property use type level but this means more work
 */
 ...
 */
select bbl , property_id, energy_star_score
from feed_ll84_bbl A
