...
public final class CSVFileLoader {
    ...
    for (CSVRecord record : csvParser) {
      if (null != record) {
        ...
        try {
          ...
          ll84FeedData.setNYCBIN(parseIntoString(record, "NYC Building Identification Number (BIN)"));
          ll84FeedData.setAddress1SelfReported(parseIntoString(record, "Address 1 (self-reported)"));
          ll84FeedData.setAddress2SelfReported(parseIntoString(record, "Address 2 (self reported)"));
          ...
          ll84FeedData.setDofGrossFloorArea(parseIntoBigDecimal(record, "DOF Gross Floor Area (ft²)"));
          ...
          ll84FeedData.setLargestPropertyUseType(parseIntoString(record, "Largest Property Use Type"));
          ...
          ll84FeedData.setDistrictSteamUse(parseIntoBigDecimal(record, "District Steam Use (kBtu)"));
          ...
          ll84FeedData.setNaturalGasUseTherms(parseIntoBigDecimal(record, "Natural Gas Use (therms)"));
          ...
          ll84FeedData.setGenerationDate(parseIntoDate(record, "Generation Date"));
  ...
  private AcrisEntry constructAcrisEntry(CSVRecord record, int iteration) throws ParseException {
    ...
    acrisEntry.setDocumentRecordedTimestamp(parseAcrisDate(record, iteration + " recorded_datetime"));
