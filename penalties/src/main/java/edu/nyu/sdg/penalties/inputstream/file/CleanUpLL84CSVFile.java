...
public final class CleanUpLL84CSVFile {
  ...
  public void loadCSV(String sourceFile, String outputFile) throws Exception {
    checkArgument(isNotBlank(sourceFile), "sourceFile is required and missing.");
    checkArgument(isNotBlank(outputFile), "outputFile is required and missing.");
      ...
      if (null != record) {
        ...
        try {
          ...
          if ( genDateFromCollection == null || genDateFromCollection.compareTo(genDateCurrentRecord)<0) {
            ...
            record.iterator().forEachRemaining(s -> sb.add(s));

          } else {
            errorCounter++;
            LOG.error("Duplicate BBL-{} RecentDate-{} DateInCollection-{}", bblCurrentRecord, genDateCurrentRecord, genDateFromCollection);
          }
