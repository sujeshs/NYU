...
public final class SOANAFileLoader {
  ...
  public SOANAFileLoader(FlowOrchestrator flowOrchestrator) {
    this.flowOrchestrator =
        requireNonNull(flowOrchestrator, "flowOrchestrator is required and missing.");

    String csvHeader = "BBL,Mail_Sequence,Mail_Recipient_Type,Borough,Block,Lot,Easement,Tax_Class,Building_Class,Owner_Name_1,Owner_Name_2,Prop_Housenum,Prop_Street,Prop_Unit,Prop_City,Prop_State,Prop_Zip,Prop_Chg_Date,Mortgage_Service_Co,Govt_Owned,Mail_Recipient_Name,Mail_CareOf,Mail_Address_1,Mail_Address_2,Mail_City,Mail_State,Mail_Zip,Mail_Country,Mail_Country_CD,Recip_Chg_Date,Mail_Chg_Date,Own_Chg_Date,Own2_Chg_Date,Phone,Email,USPS_Verified,Returned_Mail,USPS_Address";
    ...
    for (CSVRecord record : csvParser) {
      if (null != record) {
        ...
        try {
          ...
          soanaFeedData.setMail_Recipient_Type(parseIntoString(record,"Mail_Recipient_Type"));
          ...
          soanaFeedData.setMortgage_Service_Co(parseIntoString(record,"Mortgage_Service_Co"));
          soanaFeedData.setGovt_Owned(parseIntoString(record,"Govt_Owned"));
          soanaFeedData.setMail_Recipient_Name(parseIntoString(record,"Mail_Recipient_Name"));
